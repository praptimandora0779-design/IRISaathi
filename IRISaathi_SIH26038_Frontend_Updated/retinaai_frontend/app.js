...
function bind() {
    ...
    $('loginForm').onsubmit = e => { e.preventDefault(); const id = $('loginProfileId').value.trim(); const pw = $('loginPassword').value; const pid = $('loginPatientId').value.trim(); const profiles = getProfiles(); let p = profiles.find(x => x.profileId === id && x.password === pw); if (id === DEMO_PROFILE.profileId && pw === DEMO_PROFILE.password) p = DEMO_PROFILE; if (!p) { toast('Profile ID or password is incorrect'); return } if (!pid) { toast('Patient ID is required for every screening'); return } state.profile = p; state.patientId = pid; localStorage.setItem('retinaai_session', JSON.stringify({ profileId: p.profileId, patientId: pid })); openApp(); toast('Login successful') };
    document.querySelectorAll('.nav-item').forEach(b => b.onclick = () => goPage(b.dataset.page)); document.querySelectorAll('[data-go]').forEach(b => b.onclick = () => goPage(b.dataset.go)); $('logoutBtn').onclick = () => { localStorage.removeItem('retinaai_session'); state.profile = null; showAuth('choice'); $('appScreen').classList.add('hidden'); $('authScreen').classList.remove('hidden'); toast('Signed out') };
    $('browseBtn').onclick = () => $('imageInput').click(); $('dropZone').onclick = e => { if (e.target.closest('button')) return; $('imageInput').click() }; $('imageInput').onchange = e => { const f = e.target.files[0]; if (f) { $('fileName').textContent = f.name; $('fileSize').textContent = (f.size / 1024 / 1024).toFixed(2) + ' MB'; const r = new FileReader(); r.onload = () => updateImage(r.result); r.readAsDataURL(f) } }; $('dropZone').ondragover = e => { e.preventDefault(); $('dropZone').style.borderColor = '#4edbff' }; $('dropZone').ondragleave = () => { $('dropZone').style.borderColor = '' }; $('dropZone').ondrop = e => { e.preventDefault(); const f = e.dataTransfer.files[0]; if (f && f.type.startsWith('image/')) { const r = new FileReader(); r.onload = () => { $('fileName').textContent = f.name; $('fileSize').textContent = (f.size / 1024 / 1024).toFixed(2) + ' MB'; updateImage(r.result) }; r.readAsDataURL(f) } }; $('removeImage').onclick = e => { e.stopPropagation(); $('imageInput').value = ''; $('uploadPreview').classList.add('hidden'); $('uploadEmpty').classList.remove('hidden'); $('analyzeBtn').disabled = true; state.imageData = '' }; $('analyzeBtn').onclick = runPipeline;
    $('changePatientBtn').onclick = () => { const pid = prompt('Enter the Patient ID for this screening record:', state.patientId); if (pid && pid.trim()) { state.patientId = pid.trim(); setProfileUI(); refreshReportFields(); toast('Patient ID updated for this screening') } };
